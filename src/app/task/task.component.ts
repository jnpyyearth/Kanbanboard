...
export class TaskComponent implements OnInit {
  ...
  } //รับข้อมูลค่าพารามิเตอร์ของบอร์ดไอดี
  ...
  // ฟิลเตอร์ Tasks เฉพาะ Board และ Column ที่เกี่ยวข้อง
  filterTasksByColumn() {
    this.filteredTasks = {};
    ...
    if (storedTasks) {
      ...
      this.columns.forEach((column) => {
        this.filteredTasks[column.id] = this.tasks.filter(
          (task) => task.courseId === column.id && task.boardId === this.boardId
        );
      });
    }
  ...
  deleteColumn(columnId: number) {
    ...
    if (storedBoards) {
      ...
      if (boardIndex !== -1) {
        this.boards[boardIndex].columns = this.boards[
          boardIndex
        ].columns.filter((col: { id: number }) => col.id !== columnId);
        localStorage.setItem('boards', JSON.stringify(this.boards));
        this.getBoardsData();
      }
    }
  ...
  saveColumnEdit() {
    if (this.editedColumnId !== null && this.editedColumnName.trim() !== '') {
      ...
      if (storedBoards) {
        ...
        if (boardIndex !== -1) {
          let columnIndex = this.boards[boardIndex].columns.findIndex(
            (col: { id: number }) => col.id === this.editedColumnId
          );
          if (columnIndex !== -1) {
            this.boards[boardIndex].columns[columnIndex].name =
              this.editedColumnName;
            localStorage.setItem('boards', JSON.stringify(this.boards));
            this.getBoardsData();
